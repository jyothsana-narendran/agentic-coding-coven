...
async def run_pipeline(payload: CareerPipelineRequest, user_id: str = Depends(current_user_id)):
    ...
    if get_settings().supabase_url and get_settings().supabase_service_role_key:
        ...
        match = await supabase_insert('job_matches', {'user_id': user_id, 'job_target_id': target['id'], **result['job_match']})
        recommendation = await supabase_insert('recommendations', {'user_id': user_id, 'category': 'skills', 'title': 'Career recommendations', 'content': 'Prioritized actions generated by the Recommendation Agent.', 'evidence': result['recommendations'], 'status': 'pending'})
